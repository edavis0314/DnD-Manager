...
namespace SP_Generator
{
	class RunSPCSharp
	{
		...
		private static StringBuilder GeneratePullObjects(DataRow index, DataTable column)
		{
			...
			objects.AppendLine("		public static void PullData_ " + CapitalFirstLetter(index["TableName"].ToString()) + "(" + CapitalFirstLetter(index["TableName"].ToString()) + " myObj, bool recordPull)");
			objects.AppendLine("		{");
			objects.AppendLine("			using (SqlConnection connection = new SqlConnection(CreateDBConns(\"ticket\")))");
			objects.AppendLine("			{");
			...
			objects.AppendLine("				cmd.Parameters.AddWithValue(\"@dateChanged\", myObj.DateChanged);");
			objects.AppendLine("				cmd.Parameters.AddWithValue(\"@changeUser\", myObj.ChangeUser);");
		...
		private static StringBuilder GenerateUpdateObjects(DataRow index, DataTable column)
		{
			...
			objects.AppendLine("		public static void UpdateData_ " + CapitalFirstLetter(index["TableName"].ToString()) + "(" + CapitalFirstLetter(index["TableName"].ToString()) + " myObj)");
			objects.AppendLine("		{");
			objects.AppendLine("			using (SqlConnection connection = new SqlConnection(CreateDBConns(\"ticket\")))");
			objects.AppendLine("			{");
